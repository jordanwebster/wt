...
$WT_HOME/                                 --home > WT_HOME env > ~/.wt; exported as WT_HOME in doors; 0700
  ...
  repos/<label>/hub.git, repos/<label>/canonical   a `clone`d repository: the bare hub and wt's detached canonical checkout (`--path` overrides the parent)
...
LabelRec  := { path (canonicalised), gitdir_id, common_gitdir, registered_at, trees_dir|null, default_branch|null, owner: "user"|"wt" }
...
Tool    := { lockfile?: [FileName], sniff?: [ { file, toml_key?, contains? } ], requires?: Binary, sync_inputs?: [FileName], seed?: [RelPath], sweep?: { kind: "cargo", dir: RelPath }, env?, task? }
```
...
† only when `package.json` declares the script. Nudges: `node: if npm → pnpm`; `python: if pip|poetry → uv`; `cargo: sccache, used_if_env = ["RUSTC_WRAPPER=sccache"], used_if_file = [{~/.cargo/config.toml, build.rustc-wrapper ∋ sccache}]`; doctor evaluates `used_if_env` against the effective door env and each `used_if_file` rule (a sniff over a machine config file, `~/` via HOME, dotted `toml_key` walk, `contains` against the value) against the file's content → `ACCELERATOR_INACTIVE` (warn) / `ACCELERATOR_AVAILABLE` / `ACCELERATOR_MISSING` (info); never applied (R7). R7 mechanisms applied: worktree object sharing and `wt list --disk`. **Adapters own per-ecosystem cheapness (A46, A53, A79, A80)** and express it as ordinary layer-0 configuration plus the two adapter-only keys `seed` and `sweep`. Neither cargo tool sets a cargo directory variable: a tree's whole build lives in its own `target/`, never shared between trees (A64) — Cargo's unit hashes omit the workspace path, so two checkouts of one workspace would write the same slots and corrupt each other's build-script output and mtime freshness; cross-tree reuse belongs to content-addressed layers (the sccache nudge, `CARGO_HOME`). Build output dies with the tree at `git worktree remove`. pnpm uses its content-addressed store and uv its global cache. Repository and user `env` layers override the cargo value by the ordinary merge rule, and the door reports inherited values it displaces. An adapter *may* contribute `commands`, and the merge and delete rules treat such a contribution like any other layer-0 key; no built-in adapter declares any today, because the names live in a manifest rather than in the static catalog, so a repository declares its own. `wt` is never a legal claim (§5.6): a name that refuses until built would make `wt build` unrunnable.

`seed` names directories a new tree clones copy-on-write from the canonical checkout at `new` S3 (§11.8). cargo seeds `target/debug/.fingerprint`, `target/debug/build` and `target/debug/deps` — the canonical's compiled dependencies — and never the uplifted binaries beside them, so a tree only runs what it linked itself (§9.2a), nor `incremental/`, whose session directories rustc keys by crate metadata that embeds the workspace path, so a tree can never reuse the canonical's. `sweep` names the build directory wt reclaims superseded units from after every build it launches and in `prune`, and the strategy that knows the layout (§11.9); cargo sweeps `target`.
...
```
`--from` bare `X`: `refs/heads/X` → `refs/remotes/origin/X` → rev; both present and different → local wins with `FROM_LOCAL_SHADOWS_REMOTE`; `origin/X` forces remote. **Default start: local `<default>`, brought to origin's tip first (A81).** After the bounded fetch, when `origin/<default>` is ahead of local `<default>`, the local branch is fast-forwarded — by moving the ref when no worktree holds the branch (the clone model, §11.6), by `merge --ff-only` in the canonical when the canonical holds it and has no modified tracked file — and the tree branches from it, tracking it (`ls` then reads ahead/behind against the one ref that means "default"). When the branch cannot be fast-forwarded — the canonical has local changes, or another worktree holds the branch — the start is `origin/<default>` and info notice `DEFAULT_NOT_UPDATED` says so; when local `<default>` has commits origin lacks, they are part of the start and `DEFAULT_DIVERGED` (info) says so — nothing is ever reset. `--no-fetch` starts from local `<default>` as it stands; a repository without `origin/<default>` starts from local `<default>`, and one without a local `<default>` from `origin/<default>`. **The default branch is never checked out by a tree**: `--branch <default>`, or a tree name equal to it without `--branch`, is `DEFAULT_BRANCH_RESERVED` (2) in either canonical model. A fetch covers only the branches the creation consumes — the requested start when it can name an origin branch, plus the default branch. Any narrow-fetch failure other than a timeout falls back once to the full `refs/heads/*` fetch (a wanted name may be a tag, a raw revision, or a renamed default), so anything resolvable before stays resolvable; when the fallback also fails, the narrow fetch's error is reported because it names the refs the creation asked for, and a timeout propagates without a second attempt. Default branch: `origin/HEAD` → `main`/`master`/`trunk` → HEAD, cached, refreshed on fetch. A pull request on a GitHub origin is first asked of `gh pr view N --json headRefName,isCrossRepository,headRepositoryOwner`, run in the canonical checkout under the fetch deadline (A78; `--no-fetch` skips it): `gh` absent → `FORGE_CLI_MISSING` (7); `gh` failing → `FORGE_CLI_FAILED` (7) carrying gh's first stderr line, with `gh auth login` as the remedy for gh's auth exit 4 and `gh auth status` otherwise; both name `--from origin/<branch>` as the way in without gh. A head that is a branch of origin resolves exactly as `--from origin/<head>`: fetched narrowly, branch `<head>` tracking `origin/<head>`, and a second address for the same PR is `BRANCH_IN_USE`. Otherwise — a fork head, another forge, `--no-fetch` — the PR ref is mirrored: refspec by origin host github `refs/pull/N/head`; gitlab `refs/merge-requests/N/head`; bitbucket `refs/pull-requests/N/from`; unknown: pull then merge-requests; fetched as `refs/wt/pr/N`, local branch `pr/N`. Default name `pr-N`. A branch that already exists locally is checked out as it is (`FROM_LOCAL_SHADOWS_REMOTE` when it is not at `origin/<head>`). `PR_BRANCH_TRACKED` (info) needs both halves: the forge named `B` itself as the PR's branch of origin, and git, read back after the add, confirms upstream `origin/<B>`. Anything else — `--detach`, a `--branch` naming another branch, a local branch without the upstream — is `PR_BRANCH_UNTRACKED` (warn) saying why and what would push to the PR (the fork owner and branch; `git push origin HEAD:<head>`; retry without `--no-fetch`; `--from origin/<branch>`); a recorded branch re-added without asking the forge is silent while it tracks `origin/<B>`. An existing address whose record names the same PR keeps its recorded branch and asks the forge only when its worktree must be re-added from a start the record cannot supply. A PR URL selects the label whose normalised origin (https or scp-style ssh, `.git` stripped) matches host and `owner/repo`; zero/many → error with remedy. `B` defaults to the label's effective `branch` (§5.2, A77): the first declared candidate whose `meta.<key>` references are all satisfied, else `<name>` when the list is exhausted or none is declared. A candidate rendering to something git will not accept as a branch name is `BRANCH_INVALID` (2), naming the candidate, the rendered value and `--branch`. `--detach` produces no branch and a PR creation's default stays the PR's head branch or `pr/N`, neither consulting `branch`. `--branch feature/x` without a name → `feature-x`. `AddSpec`: existing branch · `-b B <start>` with `--no-track` unless start is `refs/remotes/*` · `--detach`.
...
| S2 | exclude block | 5 |
| S3 | `copy` (§11.7), then `seed` (§11.8); record `materialized`; exclude | 6, 5 |
...
After step 12, a removed linked tree whose label seeds starts an anchor refresh with a fetch (§11.10): the canonical is brought to the default branch's tip and rebuilt in the background, so the next tree starts warm. It is fire-and-forget and can never fail the removal; a refresh that does not start is `ANCHOR_START_FAILED` (warn).
...
**`clone` (A81).** `wt clone <url> [--label L] [--path P]` makes a repository wt owns: a bare hub `P/hub.git` — `git init --bare`, `remote add origin`, one fetch (class `clone`), `remote set-head --auto`, a local `<default>` at `origin/<default>` that HEAD names, and no other local branch — and the canonical `P/canonical`, a worktree of the hub detached at `<default>`'s commit; then §11.6 registration with `owner: wt`. `P` defaults to `$WT_HOME/repos/<label>` and `L` to the URL's stem; a relative `P`, and a repository named by a relative filesystem path rather than a URL, resolve against the caller's directory before git runs inside the hub. A clone that fails after the hub was initialised — an unreachable remote, a fetch past its deadline — removes the hub again, so the retry the error asks for is a clean one. Idempotent: an existing hub with its canonical registers without cloning (`cloned: false`); a hub without its canonical, or a canonical without its hub, is `PATH_OCCUPIED` (5). Data: `{ url, cloned, hub } & register.data`. In this model the default branch is a ref nothing holds, kept at origin's tip by `new` (§11.2) and `anchor` (§11.10), and the canonical is moved by re-detaching it. `unregister` of a wt-owned label tears down as for any other and reports the hub and canonical as `kept` artifacts rather than deleting them: they hold every branch the label ever kept.
...
### 11.8 `seed`
Run once per incarnation at `new` S3, after `copy`, for the directories the tree's adapters declare (§6.1 `seed`); never for canonical or adopted trees, and never from configuration. Source root = the canonical checkout. Per path: source absent or not a directory → skipped; destination present → left as it is; otherwise cloned copy-on-write to the contained destination (§5.7: parents created without following symlinks, the clone placed by directory descriptor). A filesystem that cannot clone — another volume, or one without the primitive; only APFS offers a whole-directory clone today — ends the step with info notice `SEED_SKIPPED`, and no byte copy is ever attempted: a copy of build output is exactly the multi-gigabyte creation this step exists to avoid. Success is info notice `SEED_CLONED` naming the paths; a canonical with nothing to seed is `SEED_SKIPPED`. Neither outcome fails `new` — a cold tree is merely slower. The clone is a snapshot of the canonical at that moment; cargo's own freshness rules rebuild whatever differs, and a build running in the canonical meanwhile yields at worst a partial snapshot that rebuilds the same way.

### 11.9 `sweep`
The sweep reclaims build output the workspace will never read again, so a build directory — the canonical's above all, since every tree inherits it — holds one artifact per crate rather than every configuration ever built. Adapters declare it (§6.1 `sweep`: the strategy and the build directory, scoped like `seed`); cargo is the only strategy today, applied to `target`. Cargo keeps one artifact set per build configuration, never deletes a superseded one, and writes no "last used" mark on a unit it reuses, so neither directory nor file ages can say what is still read; the sweep asks cargo instead. For each profile directory under the build directory — `<profile>` and `<triple>/<profile>`, whatever holds a `.fingerprint` — a *unit* is one `.fingerprint/<package>-<hash>` directory together with everything under `deps/` and `build/` carrying that hash. The workspace's crate roots come from `cargo metadata --locked --offline` in the tool's scope: every target source path of every package in the current resolve. A unit is live when (1) its own dep-info (`deps/*-<hash>.d`, or the `.d` in its `build/` directory for a build-script compile) names a crate root in that set — a relative root resolves against the metadata's workspace root; (2) among live units of one identity — fingerprint flavour-kind-name, crate root, features, target, profile, compile kind, rustflags, config, and output kind (metadata only, library, executable) — it is the most recently compiled (`invoked.timestamp`); or (3) it has no dep-info (a build-script run) and a live unit's recorded dependencies name its fingerprint, transitively. Everything else is deleted: old dependency versions, deleted targets, a seeded canonical's workspace units, the twin a dependency or toolchain bump superseded. Feature-set variants and check-mode twins are distinct identities and both survive: each command reuses its own. Under `incremental/`, a directory untouched for fourteen days is deleted — rustc rewrites one on every use, so there age is a use record. A pass that identifies no unit at all — none compiled from any of the workspace's crate roots, a resolve with no roots, or a directory whose every unit lacks readable dep-info — refuses to delete anything: it is looking at some other checkout's output, not at a stale one. The pass holds cargo's build-directory lock (`<profile>/.cargo-lock`, exclusive, non-blocking) throughout, so it and a build exclude each other exactly as two builds do; a held lock, a failed `cargo metadata` (an out-of-date lockfile, an undownloaded package), or a refusal ends the pass with info notice `SWEEP_SKIPPED` naming why, and a pass that deleted anything reports `SWEPT` with unit and directory counts and the space freed. There are two triggers and no flag: after every build wt launches (§11.2, §11.10), and plain `wt prune`, which plans a `sweep` item for every live tree and canonical whose pass would delete something (§12).

### 11.10 `anchor`
The anchor is the label's canonical checkout: the seed every new tree clones from, and — in the register model — the user's own checkout of the default branch. `wt anchor <label> [--no-fetch]` refreshes it in one operation under the label's anchor lock (§13.1 level 0; a held lock is `refreshed: false` with info notice `ANCHOR_BUSY`, never a wait): (1) fetch the default branch (class `fetch`; `--no-fetch` skips it; a failed fetch is `FETCH_FAILED` (warn) and the refresh continues, a build of what is here being still worth having); (2) move the canonical to `origin/<default>` — a user-owned canonical moves only by fast-forward, and only while it has the default branch checked out with no modified tracked file (untracked files do not count) and origin strictly ahead; a wt-owned canonical (§11.6 `clone`) has the ref moved first, fast-forward only and only while no worktree holds the branch, and is then re-detached onto it unless a tracked file is modified; otherwise it stays, and the refresh says why: `ANCHOR_OFF_DEFAULT`, `ANCHOR_DIRTY`, `ANCHOR_DIVERGED` (info: local commits origin lacks are never reset), `ANCHOR_NOT_MOVED` (warn: git refused); (3) build, when the label has a `build` task and the recorded build does not already cover the canonical's commit (`BuildState.head` equal to HEAD with status `ok`): exactly as `wt build <label>` — status `running` → `ok`/`failed` in the canonical's `.wt/build.status`, this process as the recorded owner, log `.wt/logs/wt-setup.log`, then the sweep (§11.9); a failed build is `BUILD_FAILED` (warn) in the refresh's notices and the verb still exits 0, since `status` and `doctor` carry the failure. Data: `{ label, refreshed, fetched: bool|null, moved: {from, to}|null, head, build: "ok"|"failed"|"fresh"|"none"|"busy", swept: {units, incremental, kb}|null }`. Started by `new` (without fetch: it fetched already), `rm` and `sync` (with fetch) for labels whose adapters seed, through the §11.2 supervisor with its priority lowered, its own report written to the canonical's `.wt/logs/wt-anchor.log`; never started while the anchor lock is held. `doctor` reports a seeding canonical without a build of its commit as `ANCHOR_COLD` (info) with the verb as remedy.

## 12. Truth: `list`, `status`, `doctor`, `prune`, logs
`wt ls [label] [--probe] [--fast] [--disk] [--meta key]`, `wt status [target] [--probe]`: address, phase (§11.1), branch/detached, dirty counts, upstream ahead/behind, behind default, sync state (`ok | stale (<files>) | failed | never`, `behind <default> by N`, **`drift (<files>)`** = sync inputs changed on the default branch since the merge-base: one bounded `git diff --name-only HEAD...<default> -- <sync_inputs>` per tree, S3/A31; `--fast` skips it). "Default" here is local `<default>`, which `new` and `anchor` keep at origin's tip (A81), and `origin/<default>` only while no local branch of that name exists, session `yes|no|unknown`, agent, tree-, repo-, and machine-tied resources `{scope, task, tied_to, state, external, undeclared, last_probe, last_error}`, slot/ports (+`bound` from one bind probe per declared port, skipped by `--fast`), path; `--disk` also sizes the tree's build output — the top-level directories holding its adapters' `seed` paths, `target` for cargo (`build_kb`, null when nothing is seeded or exists, or the path is not the tree's own — a replaced directory's configuration is not consulted). The tree is walked once for both figures. Human `ls --meta key` inserts a column named `key`, empty for trees without that value; JSON always carries the complete `meta` map and is unchanged by this option. `--probe` refreshes declarations (§10.5) and runs `exists` under each displayed record's resource lock. For `ls`, each distinct `ResourceKey` is probed once per pass: tree-tied keys remain per tree, while a repo- or machine-tied record shared across displayed trees is probed once and its one result is displayed for every tree that declares it. `status` probes each displayed record once. Doctor's state-orphan scan treats `_machine.json` as the machine store, never as an orphaned tree state file.
...
|---|---|
| `STATE_ORPHAN` (info: a state file whose address has no live entry; deleted by `prune`); `REPO_PATH_MISSING`, `TREE_REPLACED` | §4.1, §4.3–4.4, §5.4, §6.1 |
...
|---|---|---|---|
| 0 | anchor (per label) | exclusive, try-only | `anchor` for its whole run — fetch, move, build, sweep (§11.10); a held lock is `ANCHOR_BUSY`, never waited for |
...
| `prune` | per item as `remove` (session kill before step 11); `--records`: step 8 only (3, 6); tombstone collection: session check, 5; `sweep` items: cargo's build-directory lock only |
| `anchor` | 0 (try), 2 for the fetch, then the `build` run as `run` does (1s inherited by nodes, 2?,3?,4?, 6) |
...
|---|---|---|---|---|
| anchor | — | non-blocking | — | `ANCHOR_BUSY` (info, exit 0) |
...
| `wt clone <url> [--label L] [--path P]` | yes | a bare hub and a detached canonical under `P` (default `$WT_HOME/repos/<label>`), then §11.6 |
...
| `wt anchor <label> [--no-fetch]` ★ | yes (a current canonical builds nothing) | §11.10 |
...
| 1 | internal | bug |
| 2 | usage | `CONFIRM_REQUIRED`, `JSON_UNSUPPORTED`, `USE_UNREGISTER`, `NO_GATE_REFUSED`, `BRANCH_INVALID`, `DEFAULT_BRANCH_RESERVED` |
...
Tree     := { target, label, name, canonical, tree_id, path, slot, geometry: {base, stride, port_base}, phase, branch|null, detached_sha|null,
              ...
              session: "yes"|"no"|"unknown", session_name, agent|null, meta: Map<String,String>, resources: [Resource], ports: [ {name, port, bound|null} ], disk_kb|null, build_kb|null }
...
| `clone` | `{ url, cloned, hub } & register.data` |
...
| `new` / `adopt` | `{ tree, created, resumed, sync: StepRep[]|null, verify: {ok, steps: StepRep[]}|null, build: {started, log, pid, head: string|null}|null }` / `{ tree, adopted, resumed }` |
...
| `anchor` | `{ label, refreshed, fetched: bool|null, moved: {from, to}|null, head: string|null, build: "ok"|"failed"|"fresh"|"none"|"busy", swept: {units, incremental, kb}|null }` |
...
Byte stability is claimed only after normalising the declared nondeterministic fields: `wt.version`, every `at`/`since`/`started`/`recorded_at`/`removed_at`, `duration_ms`, `log`, `pid`, `tree_id`, `disk_kb`, `build_kb`, `holder.since`, `last_probe.at`, `last_error.at`.
...
| §11.9 (A80) | `wt_core::sweep` unit tests cover each liveness rule and the refusal; `sweep_reclaims_superseded_units_after_a_build_and_in_prune` builds a cargo layout by hand — a superseded twin, a stale dependency version, a feature variant, a build-script pair with an orphan run, a stale and a fresh incremental directory — behind a `cargo` that answers `metadata`, and proves what `wt build` deletes and keeps, the `SWEPT` counts, prune's plan-then-apply `sweep` item, the refusal when nothing matches the workspace, and the stand-down while cargo's lock is held | U, C |
| §11.10 (A80) | `anchor_fast_forwards_a_clean_canonical_and_records_its_build` proves fast-forward, `head` in the recorded build, `fresh` on a second run, each stay reason, `ANCHOR_BUSY` under a held lock, `NOT_FOUND`, and `ANCHOR_COLD`; `lifecycle_verbs_refresh_the_anchor_only_for_labels_that_seed` observes the detached refresh `new` (no fetch) and `rm` (fetch) start and that a non-seeding label starts none | C |
| §11.2, §11.6, §11.10, §12 (A81) | `clone_makes_a_hub_and_a_detached_canonical_that_new_branches_from` proves the hub's shape, the detached canonical, `owner: wt`, idempotence, `DEFAULT_BRANCH_RESERVED`, a tree branching from and tracking the updated local default while the canonical waits for `anchor` to re-detach it, the dirty stay with the ref still moved, `behind_default` against local `<default>`, and `unregister` keeping the hub; `new_brings_the_local_default_branch_to_origin_before_branching` proves the in-place fast-forward of a clean user canonical, `DEFAULT_NOT_UPDATED` with the `origin/<default>` start, `DEFAULT_DIVERGED` with the local start and nothing reset, and `--no-fetch` | C |
...
| The sweep asks cargo what the workspace resolves to and keeps the newest of identical units; it never judges by age | cargo marks nothing it reuses, so age would delete live artifacts (A80) |
| The anchor moves a user-owned canonical only by fast-forward, and only when clean and on the default branch | it is the user's checkout; wt never resets it (A80) |
| The default branch is a ref no tree checks out, and every creation branches from it after bringing it to origin's tip | one meaning of "default", and no way to work on it by accident (A81) |
...
| §11.2 B, §11.3, §11.4, §12, §13.1 | §11.10 | when a refresh starts, what it may move, the anchor lock |
| §6.1, §11.2 B, §11.10, §12 | §11.9 | the sweep's liveness rules and its two triggers |
| §4, §4.1, §11.2, §11.5, §11.10, §12, §14.1 | §11.6 `clone` | the hub layout, `owner`, the never-checked-out default branch |
