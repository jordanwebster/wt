...
`seed` names directories a new tree clones copy-on-write from the canonical checkout at `new` S3 (§11.8). cargo seeds `target/debug/.fingerprint`, `target/debug/build`, `target/debug/deps` and `target/debug/incremental` — the canonical's compiled dependencies — and never the uplifted binaries beside them, so a tree only runs what it linked itself (§9.2a). No cargo directory variable is set: a tree's whole build lives in its own `target/`, because Cargo's unit hashes omit the workspace path and two checkouts sharing one build directory overwrite each other's units.
